...
class FirebaseAuthService {
  ...
  private async _initialize(): Promise<void> {
    try {
      ...
      try {
        // Since we removed redux-persist, just check current Redux state
        const reduxState = store.getState();
        reduxStateRestored = !!reduxState.auth.user;
        
        CrashLogger.logAuthStep('Redux state check completed', { 
          userPresent: reduxStateRestored,
          userId: reduxState.auth.user?.uid
        });
      } catch (stateError) {
        CrashLogger.recordError(stateError as Error, 'STATE_CHECK_ERROR');
        console.warn('Redux state check failed, continuing with token restoration:', stateError);
      }
            ...
            if (this.lastProfileLoadUserId !== firebaseUser.uid && !this.isProfileLoading) {
              this.profileLoadingTimeout = setTimeout(async () => {
                try {
                  ...
                  // Profile loading completed - no need to persist since we use TokenAuthService now
                  CrashLogger.logAuthStep('Profile loading completed', { 
                    fulfilled: loadUserProfile.fulfilled.match(profileResult),
                    rejected: loadUserProfile.rejected.match(profileResult)
                  });
                } catch (profileError) {
            ...
            try {
              await Promise.race([
                Promise.all([
                  ...
                  // No need to persist since we use TokenAuthService now - just reset Redux state
                  Promise.resolve(store.dispatch(resetAuthState()))
                ]),
                new Promise<void>((_, reject) => 
